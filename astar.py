...
def get_inputs():
    parser = argparse.ArgumentParser(description='Finds the path from start to goal using A-star algorithm',usage='\t\t\tpython %(prog)s  -xs 20 -ys 15 -ths 5 -xg 300 -yg 120 -thg 6 -stp 2\t\t\tSilent Operation\n\t\t\tpython %(prog)s -xs 20 -ys 15 -ths 5 -xg 300 -yg 120 -thg 6 -stp 2 --output\t\tVerbose Operation ',)
    ...
    parser.add_argument('-stp', type=np.uint8, default=5, help="The minimum step size the robot can take")
